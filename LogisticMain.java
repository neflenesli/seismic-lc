...
public class LogisticMain {
    ...
    public static void learn(String filename, int nsteps, double alpha) throws IOException {
        List<Example> examples = fileReader("learn/lc/examples/" + filename);
        int no_inputs = examples.get(0).inputs.length;
        LogisticClassifier l = new LogisticClassifier(no_inputs);

        if (alpha > 0) {  //if the alpha is specified
            l.train(examples, nsteps, alpha);
        } else { //if the alpha isn't specified (decaying alpha)
            l.train(examples, 100000, new DecayingLearningRateSchedule());
        }
    }
